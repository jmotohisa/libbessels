...
pkgdatadir = $(datadir)/libbessels
pkgincludedir = $(includedir)/libbessels
pkglibdir = $(libdir)/libbessels
pkglibexecdir = $(libexecdir)/libbessels
...
HEADERS = $(include_HEADERS) $(libbessels_HEADERS)
...
PACKAGE_STRING = libbessels 0.1a
PACKAGE_TARNAME = libbessels
PACKAGE_URL = 
...
install-libbesselsHEADERS: $(libbessels_HEADERS)
	@$(NORMAL_INSTALL)
	@list='$(libbessels_HEADERS)'; test -n "$(libbesselsdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(libbesselsdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(libbesselsdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	...
	while read files; do \
	  echo " $(INSTALL_HEADER) $$files '$(DESTDIR)$(libbesselsdir)'"; \
	  $(INSTALL_HEADER) $$files "$(DESTDIR)$(libbesselsdir)" || exit $$?; \
	done
...
uninstall-libbesselsHEADERS:
	...
	@list='$(libbessels_HEADERS)'; test -n "$(libbesselsdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(libbesselsdir)'; $(am__uninstall_files_from_dir)
...
install-data-am: install-includeHEADERS install-libbesselsHEADERS
