...
C      C2R = STR
  ...
  160 CONTINUE
      RETURN
      END
C$$$      SUBROUTINE XERROR(MESS,NMESS,L1,L2)
C$$$C
C$$$C     THIS IS A DUMMY XERROR ROUTINE TO PRINT ERROR MESSAGES WITH NMESS
C$$$C     CHARACTERS. L1 AND L2 ARE DUMMY PARAMETERS TO MAKE THIS CALL
C$$$C     COMPATIBLE WITH THE SLATEC XERROR ROUTINE. THIS IS A FORTRAN 77
C$$$C     ROUTINE.
C$$$C
C$$$      INTEGER NMESS, L1, L2, NN, NR, K, I, KMIN
C$$$      CHARACTER*(*) MESS
C$$$      NN=NMESS/70
C$$$      NR=NMESS-70*NN
C$$$      IF(NR.NE.0) NN=NN+1
C$$$      K=1
C$$$      PRINT 900
C$$$  900 FORMAT(/)
C$$$      DO 10 I=1,NN
C$$$        KMIN=MIN0(K+69,NMESS)
C$$$        PRINT *, MESS(K:KMIN)
C$$$        K=K+70
C$$$   10 CONTINUE
C$$$      PRINT 900
